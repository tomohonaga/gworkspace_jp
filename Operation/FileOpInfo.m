...
{  
  return NSRunAlertPanel(nil, message,
             //NSLocalizedString(message, @""), 
			 NSLocalizedString(@"Ok", @""), 
                         nil, 
                         nil);
      ...
      if (samename)
        {
          ...
          if ([type isEqual: NSWorkspaceMoveOperation])
            {	
              msg = _(@"Some items have the same name;\ndo you want to replace them?");
              title = _(@"Move");		
            }
          else if ([type isEqual: NSWorkspaceCopyOperation])
            {
...
{  
  ...
  result = [fileOp requestUserConfirmationWithMessage: msg title: _(@"Error")];
    ...
    if ([files count])
      {
        if (found)
        ...
        else
          {
            ...
            [fileOp showErrorAlertWithMessage: _(@"File Operation Error!")];
            [fileOp endOperation];
            [fileOp cleanUpExecutor];
          }
      }
